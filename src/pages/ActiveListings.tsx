...
const activeProjects = sortByListedDateDesc(
  currentProjects.filter((p) => p.status === "Active Listing"),
);
...
const ActiveListings = () => {
  ...
  const visible = activeProjects.slice(0, visibleCount);
  const hasMore = visibleCount < activeProjects.length;

  return (
    <main className="min-h-screen bg-background">
      <GlobalNav />
      <SEO title={"Active Listings — Ocean City Luxury Homes"} description={"Custom luxury homes currently for sale in Ocean City, NJ by OCDG."} path="/developments/active-listings" />
      ...
      <section className="relative h-[60vh] min-h-[400px] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-4 animate-fade-in-up">
          ...
          <h1 className="heading-display text-white">Active Listings</h1>
        </div>
      </section>
